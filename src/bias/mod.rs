...
use crate::math::math::{
    apply_exponent, apply_precision, bound_below_signed, bound_signed, mul_div,
};
...
pub struct Bias {
    ...
    pub last_time_updated_borrowing_fee: u64,
}
...
impl Bias {
    ...
    pub fn net_traders_pnl(&self, price: u128) -> i128 {
        let pnl_of_longs = self.longs.traders_pnl_for_specific_bias(price, true);

        let pnl_of_shorts = self.shorts.traders_pnl_for_specific_bias(price, false);
...
impl BiasDetails {
    ...
    /// bounded above by net_reserve as that as that is the max the house can lose
    ...
    pub fn update_cumulative_funding_factor(&mut self, delta_cfr: i128) {
        ...
        // Update the next funding factor
    ...
    pub fn update_cumulative_borrowing_factor(&mut self, delta_cfr: u128) -> u128 {
        let Self {
            next_borrowing_factor: current_borrowing_factor,
            ..
        } = *self;

        let value = apply_precision(current_borrowing_factor, self.total_open_interest);
