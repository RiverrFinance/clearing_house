...
pub struct OpenPositionParams {
    ...
    /// The collateral amount to be locked for this position, in quote asset.
    /// Uses 20-decimal precision (e.g., 1000000000000000000000 for 0.1 quote units).
    /// The user's balance must cover this amount plus the execution fee (in quote asset).
    pub collateral: u128,
    ...
    /// The maximum acceptable price for opening the position, in quote asset terms.
    /// Uses 20-decimal precision.
    ...
    /// The reserve factor for risk management (quoted in quote asset units).
    /// Uses 20-decimal precision (e.g., 500000000000000000000 for 0.05 quote units).
    /// This factor is used in risk calculations and position sizing.
    #[serde(rename = "reserveFactor")]
    pub reserve_factor: u128,
}

impl PriceWaitingOperationTrait for OpenPositionParams {
    fn execute(&self) {
        _open_position(self);
    }
}

impl From<OpenPositionParams> for PriceWaitingOperation {
    fn from(params: OpenPositionParams) -> Self {
        PriceWaitingOperation::OpenPosition(params)
    }
}
